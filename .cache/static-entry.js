...
const { get, merge, isObject, flatten, uniqBy, flattenDeep, replace, concat } = require(`lodash`);
...
const chunkMapping = JSON.parse(fs.readFileSync(`${process.cwd()}/public/chunk-map.json`, `utf-8`));
...
export default (pagePath, callback) => {
  ...
  let scriptsAndStyles = flatten(
    [`app`, componentChunkName].map(s => {
      ...
      namedChunkGroups[s].assets.forEach(asset => chunks.push({ rel: `preload`, name: asset }));
