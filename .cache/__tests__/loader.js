...
describe(`Production loader`, () => {
  describe(`loadPageDataJson`, () => {
    ...
    const mockPageData = (path, status, responseText = ``, json = false) => {
      mock.get(`/page-data${path}/page-data.json`, (req, res) => {
        ...
        return res
          .status(status)
          .body(typeof responseText === `string` ? responseText : JSON.stringify(responseText));
      });
    };
    ...
    it(`should load a 404 page when page-path file is not a gatsby json`, async () => {
      ...
      expect(await prodLoader.loadPageDataJson(`/unknown-page/`)).toEqual(expectation);
    ...
    it(`should return an error when status is 500`, async () => {
      ...
      expect(await prodLoader.loadPageDataJson(`/error-page/`)).toEqual(expectation);
    ...
    it(`should retry 3 times before returning an error`, async () => {
      ...
      expect(await prodLoader.loadPageDataJson(`/blocked-page/`)).toEqual(expectation);
  ...
  describe(`loadPage`, () => {
    ...
    it(`should throw an error when 404 cannot be fetched`, async () => {
      ...
      try {
        ...
        expect(err.message).toEqual(expect.stringContaining(`404 page could not be found`));
      }
      expect(prodLoader.pageDb.size).toBe(0);
      expect(emitter.emit).toHaveBeenCalledTimes(0);
    });
  ...
  describe(`prefetch`, () => {
    ...
    it(`should prefetch when not yet triggered`, async () => {
      ...
      expect(prodLoader.apiRunner).toHaveBeenNthCalledWith(2, `onPostPrefetchPathname`, {
        pathname: `/mypath/`,
      });
    });
