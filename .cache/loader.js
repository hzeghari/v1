...
const loadPageDataJson = loadObj => {
  ...
  return doFetch(url).then(req => {
    ...
    if (status === 404 || status === 200) {
      ...
      return loadPageDataJson(Object.assign(loadObj, { pagePath: `/404.html`, notFound: true }));
    }
...
export class BaseLoader {
  ...
  loadPage(rawPath) {
    ...
    const inFlight = Promise.all([this.loadAppData(), this.loadPageDataJson(pagePath)])
      .then(allData => {
...
const createComponentUrls = componentChunkName =>
  window.___chunkMapping[componentChunkName].map(chunk => __PATH_PREFIX__ + chunk);
